...
async function getAppDetails(link, appLink) {
  const browser = await puppeteer.launch();
  const page = await browser.newPage();
  async function errorHandler(error) {
    console.log("🚀 error:", error);
    await db.collection("errorLinks").insertOne({ error, ...appLink });
    await browser.close();
  }
  if (!link.startsWith("https://apps.shopify.com")) {
  ...
  const appDetail = {
    ...
    subCategory: appLink?.subCategory,
  };
  console.log("🚀 ~ getAppDetails ~ appDetail:", appDetail);
  await db.collection("App").insertOne(appDetail);
}
async function main(browser, categories) {
  for (let i = 0; i < categories.length; i++) {
    const appLinks = categories[i].appLinks;
    for (let j = 0; j < appLinks.length; j++) {
      console.log(j);
      await getAppDetails(browser, appLinks[j], categories[i]);
    }
  }
}
...
(async () => {
  ...
  for (let i = 0; i < links.length; i++) {
    await getAppDetails(links[i].link, links[i]);
  }
})();
